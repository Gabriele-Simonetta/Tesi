...
datavalidation1 = np.zeros((2, 1))
datavalidation0 = np.concatenate((np.where(data == "01-06-2012 00:00"), np.where(
    data == "01-02-2013 00:00")))  # datavalidation0 = np.concatenate((np.where(data == "09-06-2013 00:00"), np.where(data == "01-04-2014 00:00")))
# datavalidation1 = np.concatenate((np.where(data == "09-06-2013 00:00"), np.where(data == "09-10-2014 00:00")))
...
onda3 = np.concatenate((np.where(data_tre == "10-10-2014 00:00"), np.where(data_tre == "18-10-2014 00:00")))
onda4 = np.concatenate((np.where(data_tre == "04-11-2014 00:00"), np.where(data_tre == "26-12-2014 00:00")))
onda5 = np.concatenate((np.where(data_tre == "22-03-2015 00:00"), np.where(data_tre == "8-04-2015 00:00")))
onda6 = np.concatenate((np.where(data_tre == "08-01-2016 00:00"), np.where(data_tre == "17-01-2016 00:00")))
onda7 = np.concatenate((np.where(data_tre == "05-11-2016 00:00"), np.where(data_tre == "10-11-2016 00:00")))
onda8 = np.concatenate((np.where(data_tre == "06-12-2017 00:00"), np.where(data_tre == "21-12-2017 00:00")))
onda9 = np.concatenate((np.where(data_t == "04-03-2018 00:00"), np.where(data_t == "27-03-2018 00:00")))
onda10 = np.concatenate((np.where(data_t == "03-04-2018 00:00"), np.where(data_t == "18-04-2018 00:00")))
onda11 = np.concatenate((np.where(data_t == "21-06-2018 00:00"), np.where(data_t == "01-12-2018 00:00")))
onda12 = np.concatenate((np.where(data_t == "29-01-2019 00:00"), np.where(data_t == "16-02-2019 00:00")))
onda13 = np.concatenate((np.where(data_t == "22-04-2019 00:00"), np.where(data_t == "03-05-2019 00:00")))
onda14 = np.concatenate((np.where(data_t == "03-05-2019 00:00"), np.where(data_t == "11-06-2019 00:00")))
onda15 = np.concatenate((np.where(data_t == "21-10-2019 00:00"), np.where(data_t == "01-11-2019 00:00")))
onda16 = np.concatenate((np.where(data_t == "03-11-2019 00:00"), np.where(data_t == "08-11-2019 00:00")))
onda17 = np.concatenate((np.where(data_t == "14-11-2019 00:00"), np.where(data_t == "13-12-2019 00:00")))
onda18 = np.concatenate((np.where(data_t == "18-12-2019 00:00"), np.where(data_t == "01-01-2020 00:00")))
treaning = 8
test = 18
reqm = np.zeros(test)
...
temp = 18    #18  # -- prendo in considerazione serie temporali di 18 ore-- 21 #
# -- Cicli for--#
...
X_val = pd.DataFrame(np.zeros((int(
    (datavalidation0[1, 0] - datavalidation0[0, 0]) + (datavalidation1[1, 0] - datavalidation1[0, 0])),
                               column * 3))).to_numpy()
Y_val = pd.DataFrame(np.zeros((int(
    (datavalidation0[1, 0] - datavalidation0[0, 0]) + (datavalidation1[1, 0] - datavalidation1[0, 0])), 1))).to_numpy()
...
for neu in sec:
    for ciclo in range(1, niter, 1):
        ...
        model.fit(X_tr, Y_tr, validation_data=(X_val, Y_val), epochs=100, batch_size=batch,
        ...
        onda = {}
        k = 0
        for k in range(treaning + 1, test+1, 1):
            exec(f'onda = onda{k}')
            j = 0
            ordinate1 = np.zeros(onda[1, 0] - onda[0, 0])
            ordinate2 = np.zeros(onda[1, 0] - onda[0, 0])
        ...
        mederr[r] = st.mean(maxerror[range(treaning, test, 1)])
        acc[r] = accuracy
        fin[r] = shift
        cicl[r] = ciclo
        r = r + 1
    ...
    for t in range(1, test+1, 1):
        ...
        ax = plt.gca()
        table_data = [
            ["Finestra(h):" + str(temp) + " Batch:"+str(batch)+"_Neu:" + str(neu), " Media [m]", "2 std [m]", "rmse [m]",
             "errore picchi [m]"],
        ...
        table.scale(1, 1)
        plt.savefig("Onda_Col" + str(t) + "_prediz(h)" + str(shift) + "Finestra(h)" + str(temp) + '_Batch'+ str(batch)+'_Neu' + str(neu) +'.png',
                    dpi=300, bbox_inches='tight')
        plt.show()
        # -- Save onda in txt mode --#
        ondaa = "Onda_Col" + str(t) + "_prediz(h)" + str(shift) + "Finestra(h)" + str(temp) + "_Batch"+ str(batch) +"_Neu" + str(neu) + ".txt"
    ...
    t = str((end - start) / 60)
    print('tempo:' + t + ' [min] _  neu:'+str( neu)) # -- stampa a schermo il tempo impiegato--#
